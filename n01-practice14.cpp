...
int main() {
	...
	string a = "inozemtsev nikita";
	mas tab = Tablein(a);
	cout << "Таблица частот встречаемости символов:" << endl;
	for (int i = 0; i < tab.ctr; i++) {
		cout << i + 1 << ". " << "Символ - " << tab.table[i].symbol << ", количество вхождений - " << tab.table[i].num << ", вероятность - " << tab.table[i].chance << ";" << endl;
	}
	tab = TableSort(tab);
	cout << endl;
	...
	for (int i = 0; i < tab.ctr; i++) {
		numm = getCorrectSymbolCode(tab.table[i].symbol, tree);
		bruh += tab.table[i].chance * pow((float(numm.size()) - srdlcd), 2);
	}
	cout << "Дисперсия: " << bruh;
}
